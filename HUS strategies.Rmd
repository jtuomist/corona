...
if(TRUE) { # If TRUE, use preprocessed data.
  ...
  reina <- rbind(
    read.csv("~/Downloads/reina-runs/reina_default.csv"),
    read.csv("~/Downloads/reina-runs/reina_hammer-and-dance.csv"),
    read.csv("~/Downloads/reina-runs/reina_looser-restrictions-to-start-with.csv"),
    read.csv("~/Downloads/reina-runs/reina_mitigation.csv"),
    read.csv("~/Downloads/reina-runs/reina_summer-boogie.csv")
  )
...
tmp <- out[tst0 & out$date==target & out$Restriction %in% c(0.1,0.8) & out$variable=="recovered",]
tmp$value <- tmp$value/1645000
tmp

out[tst0 & out$date==target & out$Restriction %in% c(0,0.1,0.35,0.8) & out$variable %in% nums,]
...
tmp <- out[tst0 & out$Restriction==0.41 & out$variable %in% c("dead","infected"), ]
...
tmp <- reina[reina$variable %in% c("infected","dead","hospitalized"),]
tmp <- rbind(
  cbind(
    Parameter = "lower",
...
rdi <- data.frame(
  Scenario = c(
    "default", # current restrictions
    "hammer-and-dance", # hybrid (suppression)
    "looser-restrictions-to-start-with", # Swedish path
...
print(ggplot(data=tmp[tmp$scenario==i,], aes(x=date, colour = variable, fill=variable, y=value.mean, ymin=value.lower, ymax=value.upper))+geom_ribbon(colour="white",alpha=0.1)+geom_line()+
  scale_y_log10(labels=comma)+
  #  geom_text(data=rdi[rdi$Scenario==i,], aes(x=x, y=y, label=paste("RDI:",RDI)))+
  thlTheme(show.grid.x =TRUE, legend.position = "bottom")+
  coord_cartesian(ylim=c(9,100000))+
  labs(
    title = paste("Scenario", rdi$label[rdi$Scenario==i], "with randomness"),
