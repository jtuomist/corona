...
if(TRUE) { # If TRUE, use preprocessed data.
  ...
  reina <- read.csv("~/Downloads/reina_default.csv")
  reina$date <- as.Date(reina$date)

  ### ADD MELT HERE TO PRODUCE VARIABLE COLUMN
...
tmp <- rbind(
  cbind(
    Parameter = "lower",
    aggregate(reina[c("infected","dead","hospitalized")], by = reina[c("date","scenario")], FUN=function(x) quantile(x, 0.025))
  ),
  cbind(
    Parameter = "upper",
    aggregate(reina[c("infected","dead","hospitalized")], by = reina[c("date","scenario")], FUN=function(x) quantile(x, 0.975))
  ),
  cbind(
    Parameter = "mean",
    aggregate(reina[c("infected","dead","hospitalized")], by = reina[c("date","scenario")], FUN=mean)
  )
)

tmp <- melt(tmp, id.vars = c("Parameter","date","scenario"))
tmp$value <- pmax(1, tmp$value)

tmp <- reshape(tmp, timevar = "Parameter", v.names = "value", idvar=c("date","scenario","variable"), direction="wide")

ggplot(tmp, aes(x=date, colour = variable, fill=variable, y=value.mean, ymin=value.lower, ymax=value.upper))+geom_ribbon(colour="white",alpha=0.1)+geom_line()+
  scale_y_log10()+
  thlTheme(show.grid.x =TRUE, legend.position = "bottom")+
  labs(
...
ggplot(reina, aes(x=date, y=infected))+geom_line()+geom_smooth()
