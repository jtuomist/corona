...
deaths <- read.csv("https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv",
                   stringsAsFactors = FALSE)
...
mortality <- mortality[mortality$Confirmed>300 , ] # Small numbers are too unreliable to be useful in mortality calculations
mortality$Mortality <- mortality$Deaths / mortality$Confirmed

day100 <- confirmed[confirmed$Confirmed>=100,]
day100 <- aggregate(day100$Date, by = day100[c("Province","Country")], FUN=min)

mortality <- merge(mortality, day100)
mortality$Day100 <- (mortality$Date - mortality$x) / 24 / 3600
