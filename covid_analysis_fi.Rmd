...
tmp <- c("Helsinki","Espoo","Vantaa","Turku","Kuopio","Vaasa", "Oulu","Tampere","Jyväskylä","Pori",
         "Rovaniemi","Järvenpää")
...
tsd <- dat[dat$measure=="cases" & dat$place!="Kaikki Alueet" & 
             (dat$period=="cumulative" | (grepl("SHP",dat$place) & dat$period=="daily")) , ]
...
#  if(grepl("SHP",colnames(tsd)[i])) plot(decompose(tsd[[i]]),xlab=colnames(tsd)[i])
  trend[[colnames(tsd[i])]] <- decompose(tsd[[i]])$trend
  trend.[[colnames(tsd[i])]] <- trend[2:nrow(trend),i] - trend[1:(nrow(trend)-1),i]
}
